...
async def bring(self, user: User, message: str) -> None:
    ...
    if len(args) < 1:
        await self.highrise.send_whisper(user.id, "Usage: !bring <@username>")
        return
    elif args[0][0] != "@":
    ...
    for content in response.content:
        if content[0].id == user.id:
            if isinstance(content[1], Position):
                ...
                break
    if not your_pos:
        await self.highrise.send_whisper(user.id, f"Position not found\nProbably you're using furniture.")
        return
    await self.highrise.teleport(user_id, your_pos)
